...
class Solution {
  ...
  int findMaxForm(final List<String> strs, final int m, final int n) {
    ...
    int dfs(final int i, final int maxZeros, final int maxOnes) {
      final String key = '$i:$maxZeros:$maxOnes';

      if (memo.containsKey(key)) return memo[key]!;

      if (i == strs.length) return 0;

      final int skip = dfs(i+1, maxZeros, maxOnes);

      final int zeros = strs[i].split('0').length-1;
      final int ones = strs[i].length - zeros;
      int take = 0;
      if (zeros <= maxZeros && ones <= maxOnes) {
        take = 1 + dfs(i+1, maxZeros-zeros, maxOnes-ones);
      }
